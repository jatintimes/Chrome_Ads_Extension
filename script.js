...
var isFirstSlotRequested = true;

var slotRequestedTime;
var adRenderedTime;
var firstslotreq;
var numOfSlotsRequested = 0;
...
window.addEventListener('load', function () {
    ...
    if (insTag !== null) {
        // console.log('## <ins> tag is present on the page.');
        table.push({'Event': 'HardCoded Adsense', 'Status': 'Present'});
    } else {
        // console.log('## <ins> tag is not present on the page.');
        table.push({'Event': 'HardCoded Adsense', 'Status': 'Not Present'});
    }
...
googletag.cmd.push(function () {
    googletag.pubads().addEventListener('slotRequested', function (event) {
        ...
        if (isFirstSlotRequested) {
            var slotId = event.slot.getSlotElementId();
            firstslotreq = event.slot.getSlotElementId();
            slotRequestedTime = Date.now() - pageLoadStartTime;
            // console.log('## Page load to first ad slot requested:', slotRequestedTime + 'ms');
            // console.log('   Slot requested:', slotId);
            table.push({
                'Event': 'Slot Requested Time',
                'Status': slotRequestedTime + 'ms',
            });
            isFirstSlotRequested = false;
        }
    });

    googletag.pubads().addEventListener('slotRenderEnded', function (event) {
        if (event.slot.getSlotElementId() === firstslotreq) {
            var slotId = event.slot.getSlotElementId();
            adRenderedTime = Date.now() - pageLoadStartTime;
            // console.log('## Slot requested to first ad finish render:', adRenderedTime - slotRequestedTime + 'ms');
            var slotGRequestedTime = Date.now() - pageLoadStartTime;
            // console.log('## Slot rendered to page load:', slotGRequestedTime);
            table.push({
                'Event': 'DFP Auction Time',
                'Status': adRenderedTime - slotRequestedTime + 'ms',
            });
            table.push({
                'Event': 'First Slot Rendered Time',
                'Status': slotGRequestedTime + 'ms',
            });
            isFirstSlotRequested = false;
        }
    });
...
var isopenwrap = false;
var isaps = false;
window.addEventListener('DOMContentLoaded', function () {
    var refInterval = setInterval(function () {
        if (typeof apstag !== 'undefined') {
            // console.log("## Hello APS IS READY");
            table.push({'Event': 'APS', 'Status': 'Present'});
            clearInterval(refInterval);
        }
        if (typeof PWT !== 'undefined') {
            // console.log("## Hello PWT IS READY");
            table.push({'Event': 'OpenWrap', 'Status': 'Present'});
            setTimeout(function () {
                var xsx = Object.values(PWT.bidMap).map(obj => obj.adapters)[0];
                var openwrappartner = Object.values(xsx).map(obj => obj.adapterID);
                // console.log("## " + openwrappartner);
                table.push({'Event': 'OpenWrap Partners', 'Status': openwrappartner.join(', ')});
            }, 5000);
            clearInterval(refInterval);
        }

        if (typeof pbjs !== 'undefined') {
            table.push({'Event': 'Prebid', 'Status': 'Present'});

            try {
                setTimeout(function () {
                    var prebid_bidders = Object.values(pbjs.adUnits)[0].bids.map(obj => obj.bidder);
                    var uniqueList = [...new Set(prebid_bidders)];

                    // console.log("## " + uniqueList);
                    table.push({'Event': 'Prebid Partners', 'Status': uniqueList.join(', ')});
                }, 5000);

            } catch (error) {
                // console.log(error);
                table.push({'Event': 'Error', 'Message': error.message});
                return false;
            }
    ...
    setTimeout(function () {
        clearInterval(refInterval);
        // After table is complete, print it
        if (typeof PWT === 'undefined') {
            table.push({'Event': 'OpenWrap', 'Status': 'Not Present'});
        }
        if (typeof apstag === 'undefined') {
            table.push({'Event': 'APS', 'Status': 'Not Present'});
        }
        if (typeof pbjs === 'undefined') {
            table.push({'Event': 'Prebid', 'Status': 'Not Present'});
        }
        if (typeof googletag === 'undefined') {
            table.push({'Event': 'DFP Ads', 'Status': 'Not Present'});
        }
        console.table(table);
    }, 30000);
});
